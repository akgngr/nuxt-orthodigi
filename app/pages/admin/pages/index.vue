...
const { data: pages, refresh: refreshPages, status: pagesStatus, error: pagesError } = await useFetch<Page[]>('/api/admin/pages')
...
const items = computed<Page[]>(() => ((pages.value ?? []) as unknown as Page[]))
...
<template>
  <UDashboardPanel grow>
    <template #header>
      <AdminNavbar>
        <template #leading>
          <h1 class="text-xl font-black text-gray-900 dark:text-white uppercase italic tracking-tight">Sayfa Yönetimi</h1>
        </template>
        <template #trailing>
          <UButton icon="i-lucide-plus" label="Yeni Sayfa" color="primary" class="font-bold rounded-xl" @click="openCreateDrawer" />
        </template>
      </AdminNavbar>
    </template>
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            <thead class="bg-gray-50/50 dark:bg-gray-800/50">
              <tr>
                <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest text-left">Sayfa Başlığı</th>
                <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest text-left">URL (Slug)</th>
                <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest text-left">Bileşenler</th>
                <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest text-left">Oluşturulma</th>
                <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest text-left">İşlemler</th>
              </tr>
            </thead>
            <tbody v-if="!isLoading">
              <tr v-for="page in items" :key="page.id" class="border-b border-gray-100 dark:border-gray-800">
                <td class="px-6 py-4">
                  <div class="font-semibold text-gray-900 dark:text-white">{{ page.h1Title }}</div>
                  <div class="text-xs text-gray-400">{{ page.titleTag }}</div>
                </td>
                <td class="px-6 py-4 text-sm text-gray-500">/{{ page.slug }}</td>
                <td class="px-6 py-4">
                  <UBadge color="neutral" variant="subtle" class="rounded-full px-2 py-0.5 text-[10px] font-bold">
                    {{ page.components?.length || 0 }} Bileşen
                  </UBadge>
                </td>
                <td class="px-6 py-4 text-sm">{{ new Date(page.createdAt).toLocaleDateString('tr-TR') }}</td>
                <td class="px-6 py-4">
                  <div class="flex items-center gap-2">
                    <UButton icon="i-lucide-layout-panel-top" color="neutral" variant="ghost" size="xs" @click="openComponentDrawer(page)" />
                    <UButton icon="i-lucide-edit-2" color="neutral" variant="ghost" size="xs" @click="openEditDrawer(page)" />
                    <UButton icon="i-lucide-trash-2" color="error" variant="ghost" size="xs" @click="deletePage(page.id)" />
                  </div>
                </td>
              </tr>
            </tbody>
            <tbody v-else>
              <tr>
                <td colspan="5" class="p-8 text-center text-gray-500 italic">Yükleniyor...</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  <ClientOnly>
            ...
            <div class="p-6 space-y-8 flex-1">
              ...
              <section class="space-y-4">
                <div class="flex items-center gap-2 pb-2 border-b border-gray-100 dark:border-gray-800">
                  ...
                  <h3 class="font-semibold text-gray-900 dark:text-white uppercase tracking-wider text-sm">Temel Bilgiler</h3>
                </div>
                
                <div class="grid grid-cols-1 gap-4">
                  <UFormField label="H1 Başlığı" name="h1Title" required help="Sayfanın ana başlığı (SEO için kritik)">
                    <UInput v-model="state.h1Title" placeholder="Örn: Hakkımızda" class="w-full shadow-sm" size="md" />
                  </UFormField>

                  <UFormField label="Slug (URL)" name="slug" required help="URL yolu (Örn: hakkimizda)">
                    <UInput v-model="state.slug" placeholder="hakkimizda" class="w-full shadow-sm" size="md" />
                  </UFormField>
                </div>
              </section>
              ...
              <section class="space-y-4">
                <div class="flex items-center gap-2 pb-2 border-b border-gray-100 dark:border-gray-800">
                  ...
                  <h3 class="font-semibold text-gray-900 dark:text-white uppercase tracking-wider text-sm">Görsel ve Medya</h3>
                </div>

                <div class="grid grid-cols-1 gap-4">
                  <UFormField label="Öne Çıkan Görsel URL" name="featuredImage">
                    <UInput v-model="state.featuredImage" placeholder="/images/pages/hero.jpg" class="w-full shadow-sm" size="md">
                      <template #trailing>
                        <UIcon name="i-lucide-link" class="w-4 h-4 text-gray-400" />
                      </template>
                    </UInput>
                  </UFormField>

                  <UFormField label="Görsel Alt Metni" name="featuredImageAlt">
                    <UInput v-model="state.featuredImageAlt" placeholder="Görseli açıklayan kısa metin" class="w-full shadow-sm" size="md" />
                  </UFormField>
                </div>
              </section>
              ...
              <section class="space-y-4">
                <div class="flex items-center gap-2 pb-2 border-b border-gray-100 dark:border-gray-800">
                  ...
                  <h3 class="font-semibold text-gray-900 dark:text-white uppercase tracking-wider text-sm">Sayfa İçeriği</h3>
                </div>

                <UFormField label="Özet (Summary)" name="metaDescription" help="Arama sonuçlarında görünen kısa açıklama">
                  <UTextarea v-model="state.metaDescription" placeholder="Kısa bir özet giriniz..." class="w-full shadow-sm" :rows="3" size="md" />
                </UFormField>

                <UFormField label="İçerik (Body)" name="bodyText">
                  <template #help>
              ...
              <section class="space-y-4">
                <div class="flex items-center gap-2 pb-2 border-b border-gray-100 dark:border-gray-800">
                  <div class="p-2 bg-amber-50 dark:bg-amber-900/30 rounded-lg">
                    <UIcon name="i-lucide-settings" class="w-5 h-5 text-amber-600 dark:text-amber-400" />
                  </div>
                  <h3 class="font-semibold text-gray-900 dark:text-white uppercase tracking-wider text-sm">Gelişmiş Ayarlar</h3>
                </div>
                ...
                >
                  <template #seo>
                    <div class="space-y-4 py-4">
                      <UFormField label="Title Tag" name="titleTag" required>
                        <UInput v-model="state.titleTag" placeholder="Örn: Hakkımızda | OrthoDigi" class="w-full shadow-sm" />
                      </UFormField>

                      <UFormField label="Canonical URL" name="canonicalUrl">
                        <UInput v-model="state.canonicalUrl" placeholder="https://example.com/hakkimizda" class="w-full shadow-sm" />
                      </UFormField>
                    </div>
                  </template>
                  ...
                  <template #jsonld>
                    <div class="py-4">
                      <UFormField label="JSON-LD Şeması" name="jsonLd">
                        <UTextarea v-model="state.jsonLd" placeholder='{ "@context": "https://schema.org", ... }' class="w-full font-mono text-xs bg-gray-50 dark:bg-gray-900 shadow-inner" :rows="8" />
                      </UFormField>
                    </div>
                  </template>
                </UTabs>
    ...
    <USlideover v-model:open="isComponentDrawerOpen" title="Sayfa Bileşenlerini Yönet">
      <template #content>
        <div class="p-6 flex flex-col h-full overflow-hidden">
          <div v-if="selectedPage" class="flex-1 flex flex-col gap-6 overflow-hidden">
            <div class="flex items-center justify-between">
              <div>
                <h3 class="font-bold text-lg">{{ selectedPage.h1Title }}</h3>
                <p class="text-sm text-gray-500 italic">Bileşenleri ekleyin, düzenleyin ve sıralayın.</p>
              </div>
              <UDropdown :items="[componentTypes.map(t => ({ label: t.label, icon: t.icon, click: () => addComponent(t.value) }))]" :ui="{ width: 'w-56' }">
                <UButton label="Bileşen Ekle" icon="i-lucide-plus" color="primary" />
              </UDropdown>
            </div>

            <div class="flex-1 overflow-y-auto space-y-4 pr-2">
              <div v-if="!selectedPage.components || selectedPage.components.length === 0" class="text-center py-12 border-2 border-dashed border-gray-100 dark:border-gray-800 rounded-2xl">
                <UIcon name="i-lucide-layers" class="w-12 h-12 text-gray-200 mb-4 mx-auto" />
                <p class="text-gray-400">Henüz bileşen eklenmemiş.</p>
              </div>

              <div v-for="(comp, index) in selectedPage.components" :key="comp.id" class="group bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl border border-gray-100 dark:border-gray-800 hover:border-primary-500 transition-colors">
                <div class="flex items-center justify-between mb-2">
                  <div class="flex items-center gap-2">
                    <UBadge color="primary" variant="subtle" size="sm" class="rounded-lg">#{{ index + 1 }}</UBadge>
                    <span class="font-bold text-sm uppercase tracking-tight">{{ comp.type }}</span>
                  </div>
                  <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                     <UButton icon="i-lucide-arrow-up" color="neutral" variant="ghost" size="xs" :disabled="index === 0" @click="moveComponent(index, 'up')" />
                     <UButton icon="i-lucide-arrow-down" color="neutral" variant="ghost" size="xs" :disabled="index === selectedPage.components.length - 1" @click="moveComponent(index, 'down')" />
                     <UButton icon="i-lucide-edit" color="neutral" variant="ghost" size="xs" @click="openComponentEditor(comp)" />
                     <UButton icon="i-lucide-trash" color="error" variant="ghost" size="xs" @click="removeComponent(comp.id)" />
                   </div>
                </div>
    ...
    <USlideover v-model:open="isComponentEditorOpen" :title="selectedComponent ? `${selectedComponent.type.toUpperCase()} Düzenle` : 'Bileşen Düzenle'">
      <template #content>
        <div class="p-6 overflow-y-auto max-h-[calc(100vh-120px)]">
          <div v-if="selectedComponent" class="space-y-6">
            <!-- Hero Editor -->
            <div v-if="selectedComponent.type === 'hero'" class="space-y-4">
            ...
            <!-- Text Editor -->
            <div v-else-if="selectedComponent.type === 'text'" class="space-y-4">
              <UFormField label="İçerik (HTML)">
                <UTextarea v-model="componentEditorState.body" :rows="15" />
              </UFormField>
            </div>

            <!-- CTA Editor -->
            <div v-else-if="selectedComponent.type === 'cta'" class="space-y-4">
              <UFormField label="Başlık">
                <UInput v-model="componentEditorState.title" />
              </UFormField>
            ...
            <div v-else class="space-y-4">
              <p class="text-sm text-gray-500 mb-2">Bu bileşen türü için özel düzenleyici henüz eklenmedi. Ham JSON verisini düzenleyebilirsiniz:</p>
              <UTextarea :model-value="JSON.stringify(componentEditorState, null, 2)" @update:model-value="val => { try { Object.assign(componentEditorState, JSON.parse(val)) } catch(e) {} }" :rows="15" font-mono />
            </div>

            <div class="flex justify-end gap-3 pt-6 border-t border-gray-200 dark:border-gray-800">
              <UButton label="İptal" color="neutral" variant="ghost" @click="isComponentEditorOpen = false" />
              <UButton label="Kaydet" color="primary" :loading="isSubmitting" @click="onComponentSubmit" />
            </div>
          </div>
        </div>
      </template>
